...
module.exports = {
  ...
  async execute({ args, interaction }) {
    ...
    if (!text || text.length < 16) {
      ...
      let captcha = new AttachmentBuilder(canvas.toBuffer(), {
        name: "CAPTCHA.png",
      });
...
function rngNum() {
  return Math.floor(Math.random() * 256); //the chance of it genning 256 is almost 0% as Math.random has to input 1 for that which is almost impossible as we floor it
}
