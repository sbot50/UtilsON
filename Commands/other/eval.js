...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addStringOption((option) =>
			option.setName("code").setDescription("Code to eval. use \\n for newline.").setRequired(true)
		)
		.addStringOption((option) =>
			option.setName("language").setDescription("Coding language to use. (default is javascript-node)").setAutocomplete(true)
		),
	permissions: [],
    ...
    if (res.output > 1024) res.output = res.output.substr(1011) + "..."
		let embed = new EmbedBuilder()
			.setColor(0x1cd0ce)
			.addFields([
				{
